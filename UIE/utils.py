...
#     example 格式:
#     {
#         "prompt": "citrus##category"  # 复合prompt: 实体类型##属性/关系类型
#         "content": "文本内容",
#         "result_list": [  # 支持多值属性
#             {"text": "值1", "start": 0, "end": 2},
#             {"text": "值2", "start": 5, "end": 7}
#         ]
#     }
#     """
#     # 1. 处理复合prompt (保留##分隔符，tokenizer会正确处理)
#     prompt = example["prompt"]
#     content = example["content"]
    
#     # 2. 智能分隔符处理 (兼容不同tokenizer)
#     sep_token = tokenizer.sep_token or "[SEP]"
#     if "##" in prompt and sep_token in tokenizer.special_tokens_map.values():
#         # 将##替换为[SEP]使模型更好区分层级
#         prompt_parts = prompt.split("##")
#         prompt = f" {sep_token} ".join(prompt_parts)
...
#         padding="max_length",  # 直接填充到max_seq_len
#         return_offsets_mapping=True,
#         return_tensors="np"  # 直接返回numpy数组
#     )
    
#     # 4. 重构offset_mapping (修复原始代码的偏移bug)
#     offset_mapping = encoded_inputs["offset_mapping"][0].tolist()
#     # 重新计算内容部分的偏移 (跳过prompt和特殊token)
#     content_start_idx = 0
#     for i, (start, end) in enumerate(offset_mapping):
#         if start == 0 and end == 0 and i > 0:
#             # 找到[SEP]后的位置
#             if offset_mapping[i-1][1] > 0:
#                 content_start_idx = i
#                 break
    
#     # 5. 生成标签 (支持多值属性)
#     start_ids = np.zeros(max_seq_len, dtype="float32")
#     end_ids = np.zeros(max_seq_len, dtype="float32")
    
#     for item in example.get("result_list", []):
#         # 将原始字符偏移映射到token位置
#         item_start = item["start"]
#         item_end = item["end"] - 1  # end是开区间，需-1
...
#         # 从内容开始位置向后搜索
#         for idx in range(content_start_idx, len(offset_mapping)):
#             tok_start, tok_end = offset_mapping[idx]
#             if tok_start == 0 and tok_end == 0:  # 跳过填充
#                 continue
                
#             # 匹配起始位置
#             if start_idx == -1 and tok_start <= item_start < tok_end:
#                 start_idx = idx
                
#             # 匹配结束位置
#             if tok_start <= item_end < tok_end:
#                 end_idx = idx
#                 break
...
#                 # 计算字符重叠
#                 overlap_start = max(tok_start, item_start)
#                 overlap_end = min(tok_end, item_end)
#                 overlap_len = max(0, overlap_end - overlap_start)
#                 token_len = tok_end - tok_start
                
#                 if overlap_len > 0 and overlap_len / token_len > 0.5:
#                     if start_idx == -1:
#                         start_idx = idx
#                     end_idx = idx
...
#     # 10. 类型转换 (避免后续错误)
#     return tuple([
#         x.astype("int64") if i < 3 else x.astype("float32") 
#         for i, x in enumerate(tokenized_output)
#     ])
...
def convert_ext_examples_coupled(raw_examples,
                                 negative_ratio,
                                 prompt_prefix="情感倾向",
                                 options=["正向", "负向"],
                                 separator="##",
                                 is_train=True):
    """
    Convert coupled format data for extraction task.
    Input format: [Entity]{"type1": ["entity1", "entity2"], "type2": ["entity3"]}[Text]text content...
    Step 1: Extract entities directly from [Entity] part (no entity recognition needed)
    Step 2: For each entity, generate relation prompts and identify relation objects
    """
    import re
    
    # Relation type mapping based on entity types
    # Format: (subject_type, object_type) -> relation_type
    RELATION_TYPE_MAP = {
        ("citrus", "compound"): "contain",
        ("citrus", "compoundCategory"): "contain",
        ("by_product", "compound"): "contain",
        ("by_product", "compoundCategory"): "contain",
        ("compound", "compoundCategory"): "category",
        ("compound", "category"): "category",
        ("aroma", "aroma"): "category",  # Same type entities
        ("taste", "taste"): "category",
        ("mouth_feel", "mouth_feel"): "category",
        ("citrus", "alias"): "alias",
        ("compound", "alias"): "alias",
        ("aroma", "description"): "description",
        ("taste", "description"): "description",
        ("mouth_feel", "description"): "description",
    ...
    logger.info(f"Converting coupled format data...")
    with tqdm(total=len(raw_examples)) as pbar:
        for line in raw_examples:
            line = line.strip()
            if not line:
            ...
            try:
                ...
                if not content:
                    ...
                    content = items.get("text", items.get("data", ""))
            except json.JSONDecodeError:
                # If not JSON, treat the whole line as content
                content = line
            
            # Extract Entity and Text parts
            entity_match = re.search(r'\[Entity\](.*?)\[Text\]', content)
            if not entity_match:
                pbar.update(1)
                continue
            
            entity_json_str = entity_match.group(1)
            text_start_pos = entity_match.end()  # Position where [Text] ends
            text = content[text_start_pos:]  # Text after [Text]
            
            # Parse entity JSON
            try:
                entity_dict = json.loads(entity_json_str)
            except json.JSONDecodeError:
                logger.warning(f"Failed to parse entity JSON: {entity_json_str}")
                pbar.update(1)
                continue
            
            texts.append(text)
            
            # Step 1: Extract entities directly from [Entity] part
            entity_example = []
            entity_prompt = []
            entity_example_map = {}
            entity_map = {}  # entity_name -> {"label": type, "start": start, "end": end}
            entity_id = 0
            
            # Find all entities in the text and their positions
            for entity_type, entity_list in entity_dict.items():
                ...
                for entity_name in entity_list:
                    ...
                    while True:
                        pos = text.find(search_name, start)
                        if pos == -1:
                            # Try case-insensitive search
                            if search_name == entity_name:
                                text_lower = text.lower()
                                search_name_lower = entity_name.lower()
                                pos = text_lower.find(search_name_lower, start)
                                if pos != -1:
                                    # Find the actual case in text
                                    actual_text = text[pos:pos+len(entity_name)]
                        ...
                        entity_positions.append({
                            ...
                            "label": entity_type,
                            "start": pos,
                            "end": pos + len(search_name)
                        })
                        entity_id += 1
                        start = pos + 1
                    
                    # Use the first occurrence if found, otherwise skip
                    if entity_positions:
                        entity = entity_positions[0]
                        ...
                        entity_map[entity_name] = {
                            ...
                            "end": entity["end"],
                            "id": entity["id"],
                            "text_name": entity["name"]  # Actual text name (may differ in case)
                        }
                        ...
                        if entity_type not in entity_example_map.keys():
                            entity_example_map[entity_type] = {
                                "content": text,
                                "result_list": [result],
                                "prompt": entity_type
                            }
                        else:
                            entity_example_map[entity_type]["result_list"].append(result)
                        
                        if entity_type not in entity_label_set:
                            entity_label_set.append(entity_type)
                        if entity_name not in entity_name_set:
                            entity_name_set.append(entity_name)
            ...
            # Step 2: Generate relation examples for each entity
            subject_golden = []
            relation_example = []
            relation_prompt = []
            relation_example_map = {}
            ...
            # For each entity, generate relation prompts with all other entities
            all_entities = list(entity_map.keys())
            for subject_name in all_entities:
                subject_info = entity_map[subject_name]
                subject_type = subject_info["label"]
                subject_golden.append(subject_name)
                
                # For each other entity type, create a relation prompt
                for obj_type, obj_list in entity_dict.items():
                    if not isinstance(obj_list, list):
                        obj_list = [obj_list]
                    
                    # Determine relation type based on subject and object types
                    relation_type = None
                    if (subject_type, obj_type) in RELATION_TYPE_MAP:
                        relation_type = RELATION_TYPE_MAP[(subject_type, obj_type)]
                    elif subject_type == obj_type:
                        # Same type entities, use "category" as default
                        relation_type = "category"
                    else:
                        # Default: use object type as relation type
                        relation_type = obj_type
                    
                    for obj_name in obj_list:
                        if obj_name == subject_name:
                            continue
                        
                        # Find object entity in text
                        if obj_name in entity_map:
                            obj_info = entity_map[obj_name]
                            prompt = subject_name + "的" + relation_type
                            
                            # Convert positions from text-based to content-based
                            # obj_info["start"] and obj_info["end"] are relative to text
                            # Need to add text_start_pos to get positions in full content
                            result = {
                                "text": obj_info["text_name"],
                                "start": obj_info["start"] + text_start_pos,  # Convert to content-based position
                                "end": obj_info["end"] + text_start_pos  # Convert to content-based position
                            }
                            
                            if prompt not in relation_example_map.keys():
                                relation_example_map[prompt] = {
                                    "content": content,  # Keep the full content with [Entity]... for relation extraction
                                    "result_list": [result],
                                    "prompt": prompt
                                }
                            else:
                                ...
                                exists = False
                                for existing_result in relation_example_map[prompt]["result_list"]:
                                    if (existing_result["start"] == result["start"] and 
                                        existing_result["end"] == result["end"]):
                                        exists = True
                                        break
                                if not exists:
                                    relation_example_map[prompt]["result_list"].append(result)
                            
                            if relation_type not in predicate_set:
                                predicate_set.append(relation_type)
                            relation_prompt.append(prompt)
                            
                            inverse_negative = obj_name + "的" + relation_type
                            inverse_relation.append(inverse_negative)
