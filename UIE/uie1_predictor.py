...
class UIEPredictor(object):
    ...
    def _single_stage_predict(self, inputs):
        ...
        for batch_start in range(0, len(short_input_texts), self._batch_size):
            ...
            all_input_ids.append(np.array(input_ids, dtype="int64"))
            all_offset_maps.append(offset_maps)
            if self._multilingual:
                input_ids = np.array(
        ...
        all_offset_maps = np.concatenate(all_offset_maps)
